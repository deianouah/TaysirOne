...
const Header = () => {
  ...
  const isRtl = language === "ar" || languages[language]?.direction === "rtl";
      ...
      >
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            >
              <nav className={`flex items-center gap-1 ${isRtl ? "flex-row-reverse" : "flex-row"}`}>
                {/* Home Link */}
                <Link
                  href="/"
              ...
              <div className={`flex items-center gap-4 ${isRtl ? "flex-row-reverse" : "flex-row"}`}>
                ...
                {!isAuthenticated ? (
                  <div className={`flex items-center gap-3 ${isRtl ? "flex-row-reverse" : "flex-row"}`}>
                    <Link
                      href="/login"
                      className="px-4 py-2 rounded-lg text-sm font-medium text-gray-600 dark:text-gray-300 hover:text-emerald-600 dark:hover:text-emerald-400 transition-all duration-300"
                    >
                      {t("Landing.header.Navlinks.3")}
                    </Link>
                    <Link
                      href="/signup"
                      className="bg-gradient-to-r from-emerald-500 to-emerald-600 text-white px-5 py-2.5 rounded-xl hover:from-emerald-600 hover:to-emerald-700 transition-all duration-300 shadow-lg shadow-emerald-500/20 flex items-center group text-sm font-medium"
                    >
                      {t("Landing.header.getstart")}
                      <ArrowRight className={`w-4 h-4 ml-2 transition-transform duration-300 group-hover:translate-x-1 ${isRtl ? "rotate-180 group-hover:-translate-x-1" : ""}`} />
                    </Link>
                  </div>
                ) : (
                  <div className={`flex items-center gap-3 ${isRtl ? "flex-row-reverse" : "flex-row"}`}>
                    ...
                    <div className="relative" ref={dropdownRef}>
                      ...
                      <div
                        className={`absolute mt-2 w-56 bg-white/90 dark:bg-gray-800/90 backdrop-blur-xl border border-gray-200/80 dark:border-gray-700/80 rounded-2xl shadow-xl shadow-black/[0.08] z-50 overflow-hidden transition-all duration-300 ${
